...
namespace easdk {
    ...
    [[nodiscard]] std::string make_list_naming(const std::vector<std::string> &servers);

    class ListNaming {
    public:
        ListNaming(): list_naming_("list://") {}

        void append(const std::string &server) {
        ...
        void append(const std::vector<std::string> &servers) {
            for(auto &server: servers) {
                append(server);
            }
        }

        void append(std::vector<std::pair<std::string, std::string>> &servers) {
            for (auto &server: servers) {
                std::string ser = server.first + " " + server.second;
                append(ser);
            }
        }

        const std::string &get_list_naming() const {
            return list_naming_;
        }

    private:
        std::vector<std::string> servers_;
        std::string list_naming_;
    };
