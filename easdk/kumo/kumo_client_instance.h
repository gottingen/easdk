...
// @权重类
    ...
    private:
        ...
        int64_t _qps;
        Instance *_instance;
        mutil::BoundedQueue<kumo::client::TimeInfo> _time_q;
        // content of _time_q
    ...
    public:
        ...
        SnsConnectionPool *get_bns_pool() const {
            return _bns_pool;
        }
    ...
    private:
        ...
        mutil::BoundedQueue<int64_t> _cal_select1_avg;
        int64_t _cal_select1_avg_items[CAL_SELECT1_SIZE];
        std::atomic<int64_t> _select1_avg;

        std::atomic<int64_t> _left;
        Weight _weight;
        int32_t _index;

        //cal 99.99分位
        HeapSort _min_heap;
        std::atomic<int64_t> _quartile_value;
        std::mutex _lock;
        int64_t _total_count;
        std::atomic<int64_t> _times_for_avg_and_quartile;

        // 实例状态
        std::atomic<InstanceStatus> _status;
        ...
        std::atomic<int32_t> _total_connection_num;

        //操作connections时加读写锁
        std::vector<SmartConnection> _connections;
        std::shared_mutex _rw_lock;

        SnsConnectionPool *_bns_pool;
