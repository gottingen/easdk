...
namespace kumo::client {
    ...
    int Service::init(const Option &option) {
        ...
        if (ret < 0) {
            CLIENT_WARNING("load instance info from sns is wrong");
            return ret;
        }
    ...
    int Service::init2(const std::string &link_url, const std::string &username,
        ...
        std::vector<std::string> list_shards = turbo::str_split(list, ";");
        for (uint32_t i = 0; i < list_shards.size(); i++) {
            ...
            std::vector<std::string> instances_str = turbo::str_split(shard, ",");
            vector<InstanceInfo> instances;
            for (uint32_t j = 0; j < instances_str.size(); j++) {
                InstanceInfo info;
                info.id = instances_str[j];
                std::vector<std::string> items = turbo::str_split(info.id, ":");
                info.ip = items[0];
            ...
            SnsConnectionPool *pool = new(std::nothrow) SnsConnectionPool(
                    "ip_list",
                    "",
                    _conn_num_per_bns,
    ...
    SmartConnection Service::fetch_connection() {
        ...
        SnsConnectionPool *pool = _get_pool_no_partition_key();
        if (pool == nullptr) {
    ...
    SmartConnection Service::fetch_connection(uint32_t partition_key) {
        ...
        SnsConnectionPool *pool = _get_pool_partition_key(partition_key);
        if (pool == nullptr) {
            CLIENT_WARNING("get conn pool when fetch connection fail, service_name: %s",
                           _name.c_str());
            return SmartConnection();
    ...
    SmartConnection Service::fetch_connection_by_shard(int shard_id, const std::string &ip_port) {
        ...
        SnsConnectionPool *pool = nullptr;
    ...
    map<int, SnsConnectionPool *> Service::get_id_bns_map() const {
        return _id_bns_map;
    }
    ...
    SnsConnectionPool *Service::_get_pool_no_partition_key() {
    ...
    SnsConnectionPool *Service::_get_pool_partition_key(
            uint32_t partition_key) {
        ...
        map<int, SnsConnectionPool *>::iterator iter = _id_bns_map.find(id);
        if (iter == _id_bns_map.end()) {
            CLIENT_WARNING("there is no pool according to sns id:%d, service:%s",
                           id, _name.c_str());
            return nullptr;
        }
    ...
    SmartConnection Service::_fetch_and_select_db(
        ...
        do {
            ...
            int ret = conn->execute(sql, nullptr);
            if (ret == 0) {
                return conn;
    ...
    int Service::_load_bns_info(const std::vector<DbShardOption> &options) {
        ...
        for (auto &option: options) {
            int id = option.id;
            if (_id_bns_map.count(id) != 0) {
                CLIENT_WARNING("sns configuration repeat, bns_id: %d", id);
                return CONFPARAM_ERROR;
            }
            ...
            std::vector<std::string> instances_str = turbo::str_split(ip_list, ",");
            vector<InstanceInfo> instances;
            for (uint32_t j = 0; j < instances_str.size(); j++) {
                InstanceInfo info;
                info.id = instances_str[j];
            ...
            CLIENT_DEBUG("sns pool connection successfully, pool name: ip_list, pool id:%d", id);

        }

        CLIENT_DEBUG("load sns info successfully, sns size: %d", conf_unit["db_shard"].size());
        return SUCCESS;
    }
    ...
    void Service::_clear() {
        map<int, SnsConnectionPool *>::iterator iter1 = _id_bns_map.begin();
