...
namespace kumo::client {
    ...
    SnsConnectionPool::SnsConnectionPool(
            const string &sns_name,
    ...
    int SnsConnectionPool::init(bool connect_all, vector<InstanceInfo> *list) {
        ...
        if (_conn_conf.faulty_exit &&
            ...
            CLIENT_WARNING("sns:%s tag:%s num of instance[%d] * "
        ...
        _conf_connection_per_instance = divide_ceil(_total_conn_num, num_online_instance, nullptr);
        //初始化rolling_instance值
        //_rolling_instance = _instances_map.begin()->first;
        _rolling_num.store(0);
        if (connect_all) {
            _is_used = true;
            int num = divide_ceil(_total_conn_num, num_online_instance, nullptr);
        ...
        CLIENT_DEBUG("sns init successfully, bns_name:%s", _bns_name.c_str());
        print_conn_info();
        return SUCCESS;
    }

    SmartConnection SnsConnectionPool::fetch_connection(const std::string &ip_port) {
        _is_used = true;
        TimeCost cost;
        int try_times = 0;
        do {
    ...
    Instance *SnsConnectionPool::select_instance(const std::string &ip_port) {
        if (ip_port.empty()) {
            return nullptr;
        }

        // 加读锁
        std::shared_lock r_lock(_rw_lock);
        //选择这个ip_port上的实例
    ...
    Instance *SnsConnectionPool::_select_instance(const std::string &ip_port) {
        // 加读锁
        ...
        if (ip_port != "") {
            ...
            if (_instances_map.count(ip_port) == 1 &&
                ...
                CLIENT_WARNING("sns:%s instance %s not avaliable, rand select another",
    ...
    Instance *SnsConnectionPool::_select_instance_local_aware() {
    ...
    Instance *SnsConnectionPool::_select_instance_random() {
    ...
    Instance *SnsConnectionPool::_select_instance_rolling(bool whether_lower) {
    ...
    int SnsConnectionPool::get_table_id(
    ...
    int SnsConnectionPool::hang_check() {
        if (!_is_used) {
            return SUCCESS;
        }
    ...
    int SnsConnectionPool::healthy_check(bool need_detect_dead) {
        if (!_is_used) {
            //CLIENT_WARNING("sns pool:%s is not used", _bns_name.c_str());
            return SUCCESS;
        }
        if (_bns_info_ptr == nullptr) {
            CLIENT_FATAL("sns info ptr is null, bns_name:%s", _bns_name.c_str());
            return CONFPARAM_ERROR;
        }
        ////debug
        if (need_detect_dead) {
        ...
        std::unique_lock lock(_bns_info_ptr->mutex_lock);
        ...
        for (; iter1 != _bns_info_ptr->instance_infos.end(); ++iter1) {
            ...
            if (iter1->second.is_available) {//可用
                ...
                if (_instances_map.count(id) == 0) {
                    ...
                    // 增加实例，加写锁
                    // std::unique_lock lock(_lock);
                    std::unique_lock w_lock(_rw_lock);
                    _instances_map[iter1->second.id] = instance;
                    _instances_index.push_back(iter1->second.id);
                    CLIENT_WARNING("sns pool new instance available, bns_name:%s, instance:%s",
                ...
                //不存在，不处理
                if (_instances_map.count(id) == 0) {
                    CLIENT_DEBUG("sns syn new instance not available, bns_name:%s, instance:%s",
                                 _bns_name.c_str(), iter1->second.id.c_str());
                    continue;
                }
                Instance *instance = _instances_map[id];
        ...
        for (; iter2 != _instances_map.end(); ++iter2) {
            ...
            CLIENT_WARNING("instance not exist in sns,  bns_name:%s, instance:%s, id:%s",
    ...
    string SnsConnectionPool::get_name() const {
        return _bns_name;
    }

    int SnsConnectionPool::get_online_instance_num() {
    ...
    int SnsConnectionPool::get_total_conn_num() const {
        return _total_conn_num;
    }

    string SnsConnectionPool::get_comment_format() const {
        return _comment_format;
    }

    bool SnsConnectionPool::is_split_table() const {
        return _name_logic_db_map.size();
    }

    void SnsConnectionPool::print_conn_info() {
        //std::unique_lock lock(_lock);
        std::shared_lock r_lock(_rw_lock);
        CLIENT_WARNING("bns_name:%s, total_weight:%ld", _bns_name.c_str(), _total_weight.load());
        map<string, Instance *>::iterator iter = _instances_map.begin();
        for (; iter != _instances_map.end(); ++iter) {
            CLIENT_WARNING("Instance:%s, instance_status:%s, sns:%s, tag:%s"
    ...
    int SnsConnectionPool::alloc_connection(int conn_num) {
        ...
        int conn_num_per_instance = divide_ceil(conn_num, size, nullptr);
        map<string, Instance *>::iterator iter = _instances_map.begin();
        for (; iter != _instances_map.end(); ++iter) {
            if (iter->second->get_status() == ON_LINE) {
    ...
    void SnsConnectionPool::update_parent_weight(int64_t diff, size_t index) {
        //std::unique_lock lock(_lock);
        std::shared_lock r_lock(_rw_lock);
        while (index != 0) {
