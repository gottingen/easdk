...
#include <easdk/kumo/kumo_client_mysql_async.h>
...
namespace kumo::client {
    ...
    int MysqlConnection::connect(ConnectionConf &conn_conf, bool change_status) {
        if (_sql_handle != nullptr) {
            CLIENT_WARNING("sql handle is not nullptr");
        ...
        MYSQL *mysql = nullptr;
        do {
            _sql_handle = mysql_init(nullptr);
            if (_sql_handle == nullptr) {
    ...
    void MysqlConnection::_parse_table_name_list(const string &sql_rewrite,
        ...
        if (from_locate != string::npos) {
            ...
            vector<string> tables = turbo::str_split(sub, " ", turbo::SkipEmpty());
    ...
    int MysqlConnection::_get_db_table_name(vector<string>::iterator iter,
        ...
        turbo::trim_all(*iter);
        std::vector<string> db_table_name = turbo::str_split(*iter, ".", turbo::SkipEmpty());
        ...
        if (db_table_name.size() == 2) {
            turbo::trim_all(&db_table_name[0]);
            db_name = db_table_name[0];
            turbo::trim_all(&db_table_name[1]);
    ...
    int MysqlConnection::_process_result(bool store, ResultSet *result) {
        ...
        _trace_time_os << " store_in:" << mutil::gettimeofday_ms() % 1000000;
        MYSQL_RES *tmp_res =
                store ? mysql_async_store_result(this) : mysql_use_result(_sql_handle);
        _trace_time_os << " store_out:" << mutil::gettimeofday_ms() % 1000000;
        if (!tmp_res) {
            CLIENT_WARNING("mysql store or mysql use fail, ip:%s, port:%d",
                           _instance->get_ip().c_str(), _instance->get_port());
            return CONNECTION_QUERY_FAIL;
    ...
    int MysqlConnection::_process_query(const string &sql_rewrite) {
        TimeCost cost;
        _trace_time_os.str("");
        _trace_time_os << " in:" << mutil::gettimeofday_ms() % 1000000;
        int ret = mysql_async_real_query(this, sql_rewrite.c_str(), sql_rewrite.size(), &_trace_time_os);
        _trace_time_os << " out:" << mutil::gettimeofday_ms() % 1000000;
        if (ret == 0) {
            return SUCCESS;
        }
        int error_code = 0;
    ...
    int MysqlConnection::_add_comment(SnsConnectionPool *pool,
                                      string &comment_format) {
        ...
        if (!comment_format.empty()) {
            ...
            if (pos != string::npos && _has_partition_key) {
                comment_format.replace(pos, 1, std::to_string(_partition_key));
            }
        }
        return SUCCESS;
    }
