    ...
    void Instance::hang_check() {
        ...
        //统计实例状态是否为delay
        {
            ...
            for (size_t i = 0; i < _connections.size(); ++i) {
                ...
                //    mutil::gettimeofday_us() - _connections[i]->get_begin_time_us(),
        ...
        while (iter != _connections.end()) {
            CLIENT_WARNING("connection:%d, _begin_execute:%d, "
                           ...
                           mutil::gettimeofday_us() - (*iter)->get_begin_time_us(),
                           _quartile_value.load());
            (*iter)->kill();
...
//若pool中存在该实例，则根据pool中实例的状态，做不同的操作
        ...
        if (_status.load() == FAULTY) {
            ...
            if (ret == 0) {
                ...
                int conn_num = divide_ceil(_bns_pool->get_total_conn_num(),
                                           _bns_pool->get_online_instance_num() + 1, nullptr);
    ...
    int Instance::detect_fun(int64_t &select1_time) {
        ...
        string sql = "select 1";
        ret = conn->execute(sql, nullptr);
        select1_time = mutil::gettimeofday_us() - start_time;
        if (ret != SUCCESS) {
    ...
    void Instance::_realloc(int conn_num) {
        ...
        if (ret == INSTANCE_NOT_ENOUGH) {
            CLIENT_FATAL("there are not enough instance to add connection, instance:%s:%d, sns name:%s",
