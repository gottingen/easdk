...
namespace kumo::client {
    ...
    int fiber_wait_event(Connection *connection, int status,
                         bool *is_timeout, std::ostringstream *os = nullptr) {
        if (*is_timeout) {
        ...
        if (status & MYSQL_WAIT_TIMEOUT) {
            ...
            while (i < count && ret != 0) {
                ...
                if (os != nullptr && i < 10) {
                    *os << i << ":" << mutil::gettimeofday_ms() % 1000000 << " ";
                }
                timeout.tv_sec += 0;
                ...
                p = &timeout;
                ret = fiber_fd_timedwait(fd, event_in, p);
                ++i;
            }
            if (os != nullptr) {
                *os << "cnt:" << i << ")";
            ...
            ret = fiber_fd_timedwait(fd, event_in, nullptr);
        }
    ...
    MYSQL *mysql_async_real_connect(Connection *connection, const char *host,
        ...
        // while (status) {
        //     status = fiber_wait_event(mysql_in, status);
    ...
    int mysql_async_real_query(Connection *connection,
        ...
        int ret = 0;
        if (connection->get_mysql_handle() == nullptr) {
        ...
        try {
            ...
            while (status) {
                status = fiber_wait_event(connection, status, &is_timeout, os);
    ...
    int fiber_ping_wait_event(Connection *connection, int status) {
        timespec *p = nullptr;
        timespec timeout;
        int fd = mysql_get_socket(connection->get_mysql_handle());
        int event_in = 0;
    ...
    int mysql_async_ping(Connection *connection) {
        ...
        try {
            ...
            while (status) {
                status = fiber_ping_wait_event(connection, status);
    ...
    //    // while (status) {
    //    //     status = fiber_wait_event(mysql, status);
    ...
    MYSQL_RES *mysql_async_store_result(Connection *connection) {
        ...
        MYSQL_RES *ret = nullptr;
        int status = mysql_store_result_start(&ret, mysql);
        bool is_timeout = false;
        while (status) {
            status = fiber_wait_event(connection, status, &is_timeout);
