...
namespace easdk::sirius {
    ...
    ConfigInfoBuilder::ConfigInfoBuilder(eapi::sirius::ConfigInfo *info) : _info(info) {
        _info->Clear();
    }


    void ConfigInfoBuilder::set_info(eapi::sirius::ConfigInfo *info) {
        _info = info;
        _info->Clear();
    }
    ...
    turbo::Status ConfigInfoBuilder::build_from_file(const std::string &name, const std::string &file_path,
                                                     const eapi::sirius::Version &version,
                                                     const eapi::sirius::ConfigType &type) {
        auto lfs = alkaid::Filesystem::localfs();
        std::string content;
    ...
    turbo::Status ConfigInfoBuilder::build_from_file(const std::string &name, const std::string &file_path,
        ...
        std::string content;
        STATUS_RETURN_IF_ERROR(lfs->read_file(file_path, &content));
        eapi::sirius::Version tmp_version;
