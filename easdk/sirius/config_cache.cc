...
TURBO_FLAG(std::string, config_cache_dir, "", "config cache dir");
...
namespace easdk::sirius {
    ...
    turbo::Status ConfigCache::init() {
        ...
        _cache_dir = turbo::get_flag(FLAGS_config_cache_dir);
        if(_cache_dir.empty()) {
            return turbo::OkStatus();
        }
        std::error_code ec;
    ...
    turbo::Status ConfigCache::add_config(const eapi::sirius::ConfigInfo &config) {
        {
            std::unique_lock lock(_cache_mutex);
            if (_cache_map.find(config.name()) == _cache_map.end()) {
                _cache_map[config.name()] = std::map<collie::ModuleVersion, eapi::sirius::ConfigInfo>();
            }
    ...
    void ConfigCache::do_add_config(const eapi::sirius::ConfigInfo &config) {
            if (_cache_map.find(config.name()) == _cache_map.end()) {
                _cache_map[config.name()] = std::map<collie::ModuleVersion, eapi::sirius::ConfigInfo>();
            }
            auto it = _cache_map.find(config.name());
    ...
    /// \param config
    /// \return
    turbo::Status ConfigCache::get_config(const std::string &name, eapi::sirius::ConfigInfo &config) {
        std::unique_lock lock(_cache_mutex);
        auto it = _cache_map.find(config.name());
        if (it != _cache_map.end()) {
    ...
    turbo::Status ConfigCache::write_config_file(const std::string &dir, const eapi::sirius::ConfigInfo &config) {
        if (dir.empty()) {
            return turbo::OkStatus();
        }
        auto file_path = make_cache_file_path(dir, config);
        return easdk::Dumper::dump_proto_to_file(file_path, config);
    }

    turbo::Status ConfigCache::remove_config_file(const std::string &dir, const eapi::sirius::ConfigInfo &config) {
        if (dir.empty()) {
            return turbo::OkStatus();
        }
        auto file_path = make_cache_file_path(dir, config);
