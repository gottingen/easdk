...
namespace easdk::cli {
    ...
    make_zone_create(eapi::sirius::ZoneInfo *req);

    [[nodiscard]] turbo::Status
    make_zone_remove(eapi::sirius::ZoneInfo *req);

    [[nodiscard]] turbo::Status
    make_zone_modify(eapi::sirius::ZoneInfo *req);


    collie::table::Table show_discovery_query_zone_response(const std::vector<eapi::sirius::ZoneInfo> &res);
