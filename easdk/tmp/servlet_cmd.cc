...
namespace easdk::cli {
    ...
    void setup_servlet_cmd(turbo::cli::App &app) {
    ...
    void run_servlet_cmd(turbo::cli::App* app) {
        // Do stuff...
    ...
    collie::table::Table show_discovery_query_servlet_response(const eapi::sirius::DiscoveryQueryResponse &res) {
    ...
    turbo::Status make_servlet_create(eapi::sirius::DiscoveryManagerRequest *req) {
        eapi::sirius::ServletInfo *servlet_req = req->mutable_servlet_info();
    ...
    turbo::Status make_servlet_remove(eapi::sirius::DiscoveryManagerRequest *req) {
        eapi::sirius::ServletInfo *servlet_req = req->mutable_servlet_info();
    ...
    turbo::Status make_servlet_modify(eapi::sirius::DiscoveryManagerRequest *req) {
    ...
    turbo::Status make_servlet_list(eapi::sirius::DiscoveryQueryRequest *req) {
        req->set_op_type(eapi::sirius::QUERY_SERVLET);
        return turbo::OkStatus();
    }

    turbo::Status make_servlet_info(eapi::sirius::DiscoveryQueryRequest *req) {
        req->set_op_type(eapi::sirius::QUERY_SERVLET);
        auto rs = check_valid_name_type(ServletOptionContext::get_instance()->app_name);
        if (!rs.ok()) {
            return rs;
