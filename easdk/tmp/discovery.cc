...
#include <easdk/cli/sirius/servlet_cmd.h>
...
namespace easdk::cli {
    ...
    void DiscoveryCmd::run_discovery_list_instance_cmd() {
        eapi::sirius::DiscoveryQueryRequest request;
        eapi::sirius::DiscoveryQueryResponse response;

        ScopeShower ss;
        auto rs = make_discovery_list_instance(&request);
        PREPARE_ERROR_RETURN_OR_OK(ss, rs, request);
        rs = easdk::sirius::DiscoveryClient::get_instance()->discovery_query(request, response, nullptr);
        RPC_ERROR_RETURN_OR_OK(ss, rs, request);
        auto table = ShowHelper::show_response(response.errcode(), request.op_type(),
                                               response.errmsg());
        ss.add_table("result", std::move(table), response.errcode() == eapi::kOk);
    ...
    DiscoveryCmd::make_discovery_add_instance(eapi::sirius::DiscoveryManagerRequest *req) {
        eapi::sirius::ServletInfo *instance_req = req->mutable_servlet_info();
        req->set_op_type(eapi::sirius::OP_CREATE_SERVLET);
        if (!DiscoveryOptionContext::get_instance()->json_file.empty()) {
            auto rs = easdk::Loader::load_proto_from_file(DiscoveryOptionContext::get_instance()->json_file,
                                                               *instance_req);
            return rs;
        }
    ...
    [[nodiscard]] turbo::Status
    DiscoveryCmd::make_discovery_remove_instance(eapi::sirius::DiscoveryManagerRequest *req) {
        eapi::sirius::ServletInfo *instance_req = req->mutable_servlet_info();
        req->set_op_type(eapi::sirius::OP_DROP_SERVLET);
        if (!DiscoveryOptionContext::get_instance()->json_file.empty()) {
            auto rs = easdk::Loader::load_proto_from_file(DiscoveryOptionContext::get_instance()->json_file,
                                                               *instance_req);
            return rs;
    ...
    [[nodiscard]] turbo::Status
    DiscoveryCmd::make_discovery_update_instance(eapi::sirius::DiscoveryManagerRequest *req) {
        eapi::sirius::ServletInfo *instance_req = req->mutable_servlet_info();
        req->set_op_type(eapi::sirius::OP_MODIFY_SERVLET);
        if (!DiscoveryOptionContext::get_instance()->json_file.empty()) {
            auto rs = easdk::Loader::load_proto_from_file(DiscoveryOptionContext::get_instance()->json_file,
                                                               *instance_req);
            return rs;
    ...
    [[nodiscard]] turbo::Status
    DiscoveryCmd::make_discovery_list_instance(eapi::sirius::DiscoveryQueryRequest *req) {
        req->set_op_type(eapi::sirius::QUERY_SERVLET);
        auto opt = DiscoveryOptionContext::get_instance();
    ...
    collie::table::Table DiscoveryCmd::show_query_instance_list_response(const eapi::sirius::DiscoveryQueryResponse &res) {
        ...
        result[last].format().font_color(collie::Color::green);
        int i = 0;
        std::vector<eapi::sirius::QueryInstance> sorted_list;
        for (auto &ns: instance_list) {
            sorted_list.push_back(ns);
        }
        auto less_fun = [](const eapi::sirius::QueryInstance &lhs, const eapi::sirius::QueryInstance &rhs) -> bool {
    ...
    collie::table::Table DiscoveryCmd::show_query_instance_info_response(const eapi::sirius::DiscoveryQueryResponse &res) {
        collie::table::Table result;
